...
use fake::{
    faker::{chrono::pt_br::DateTimeBetween, internet::en::SafeEmail, name::zh_cn::Name},
    Dummy, Fake, Faker,
};
use nanoid::nanoid;
use rand::Rng;
...
async fn main() -> Result<()> {
    let pool = PgPool::connect("postgres://alon:alon123456@localhost:5432/stats").await?;
    for i in 1..=500 {
        ...
        println!("Done batch {} in {:?}", i, start.elapsed());
    }
    Ok(())
}
...
impl Dummy<IntList> for Vec<i32> {
    fn dummy_with_rng<R: Rng + ?Sized>(v: &IntList, rng: &mut R) -> Self {
        ...
        (0..size)
            .map(|_| start + rng.gen_range(start..start + len))
            .collect()
    }
}

struct UniqueEmail;
pub const ALPHABAT: [char; 36] = [
